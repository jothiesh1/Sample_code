...
WARMUP_FRAMES = 30  # Number of frames to wait before starting detection
REQUIRED_STABLE_READINGS = 10  # Number of stable readings required before alerting

# Drowsiness detection parameters
EAR_THRESHOLD = 0.25
DROWSY_FRAMES_THRESHOLD = 20
DROWSY_CONFIRMATION_FRAMES = 5
...
def compute_ear(landmarks, eye_indices):
    try:
        ...
        # Validate EAR value
        if 0.15 <= ear <= 0.45:  # Adjusted range for more stability
            return ear
        return 0
    except Exception as e:
...
def is_measurement_stable(value, stable_values, tolerance=0.1):
    """Check if the measurement is stable within tolerance"""
    if len(stable_values) < REQUIRED_STABLE_READINGS:
        return False
...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        if ear > 0:
            ...
            if not system_ready:
                stable_ear_values.append(ear)
                warmup_counter += 1
                
                if warmup_counter >= WARMUP_FRAMES:
                    if len(stable_ear_values) >= REQUIRED_STABLE_READINGS:
                        system_ready = True
                        baseline_ear = np.mean(stable_ear_values)
                        print(f"System ready! Baseline EAR: {baseline_ear:.3f}")
                    else:
                        warmup_counter = WARMUP_FRAMES - 5  # Keep checking for stability
